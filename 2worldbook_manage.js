...
$menuBtn.on("click", async () => {
  ...
  const customCSS = `
        <style>
            dialog.wb-manager-dialog { width: 92vw !important; max-width: 1600px !important; transition: zoom 0.2s ease-out; overflow: hidden; font-family: sans-serif; background: var(--SmartThemeBlurTintColor) !important; }
  ...
  const showTab = (tabName) => {
    ...
    $ui
      .find(
        "#wb-main-view, #wb-char-view, #wb-assoc-view, #wb-edit-snap-view, #wb-detailed-snap-view, #wb-bind-view, #wb-entry-view, #wb-detail-view",
      )
      .hide();
  ...
  const renderAssocView = () => {
    ...
    if (userBooks.length === 0) {
      ...
      userBooks.forEach((wb) => {
        const $item =
          $(`<div style="display:inline-flex; align-items:center; gap:8px; background:var(--SmartThemeBotMesColor); border:1px solid #339af0; padding:6px 12px; border-radius:4px; transition:0.2s;">
    ...
    if (!charName) {
      ...
      if (cBooks.length === 0) {
        ...
        cBooks.forEach((wb) => {
          ...
          const $item =
            $(`<div style="display:inline-flex; align-items:center; gap:8px; background:var(--SmartThemeBotMesColor); border:1px solid var(--SmartThemeQuoteColor); padding:6px 12px; border-radius:4px; transition:0.2s;">
  ...
  const openEntryTuneView = async (wbName, fromView = "#wb-main-view") => {
    ...
    $ui
      .find(
        "#wb-main-view, #wb-detail-view, #wb-assoc-view, #wb-tab-strip, #wb-char-view",
      )
      .hide();
  ...
  const renderEntryList = () => {
    ...
    const filteredEntries = tuneEntries.filter((entry) => {
      const searchStr =
        `${entry.name || ""} ${(entry.strategy?.keys || []).join(",")}`.toLowerCase();
    ...
    for (const groupName of sortedGroupNames) {
      ...
      gEntries.forEach((entry) => {
        ...
        const $right = $(
          '<div style="display:flex; gap:8px; margin-left:auto; flex-shrink:0;"></div>',
        );

        if (!isEntryBatchMode) {
          $right.append(
            $(
              '<button class="menu_button interactable wb-nowrap-btn" style="color:#fcc419; margin:0;" title="设置此条目的分组"><i class="fa fa-folder-tree"></i></button>',
            ).on("click", async () => {
              let res = await SillyTavern.callGenericPopup(
                `设置 [${entry.name || "未命名"}] 的分组名称\\n(同一个世界书内，名字相同就会归为一组，留空代表未分类):`,
                SillyTavern.POPUP_TYPE.INPUT,
                entry.group || "",
              );
              if (res !== null) {
                entry.group = res.trim();
                renderEntryList();
              }
            }),
          );
        ...
        $right.append(
          $(
            '<button class="menu_button interactable wb-nowrap-btn" style="color:#ff6b6b; margin:0;" title="删除条目"><i class="fa fa-trash"></i></button>',
          ).on("click", async () => {
            if (
              (await SillyTavern.callGenericPopup(
                `确认删除 [${entry.name || "未命名"}]？`,
  ...
  const openDetailEditView = (index) => {
    ...
    $ui.find("#wb-entry-view").hide();
    $ui.find("#wb-detail-view").fadeIn(200);
  };
