...
const getFloatAppearance = () => {
  return JSON.parse(
    localStorage.getItem("lulu_wb_floating_appearance") ||
      '{"iconType":"fa","iconValue":"fa-book-atlas","emoji":"📖","imgUrl":"","useThemeColor":true,"bgColor":"#2a2e33","iconColor":"#70a1ff","borderColor":"#70a1ff"}',
  );
};
...
或直接粘贴 <svg ...>...</svg> 代码" style="width:100%; box-sizing:border-box; padding:6px; margin:0; min-height:60px; resize:vertical; font-size:12px;"></textarea>
  ...
  const updateFloatAppearanceRows = () => {
    ...
    $ui
      .find("#wb-float-bg-color, #wb-float-icon-color, #wb-float-border-color")
      .prop("disabled", useTheme);
  };

  $ui.find("#wb-float-appearance-btn").on("click", () => {
    loadFloatAppearanceUI();
    $ui.find("#wb-float-appearance-panel").slideToggle("fast");
  });
  $ui
